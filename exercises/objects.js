...
export function collectByFirstLetter(...words) {
  ...
  words.forEach(element => {
    if (result[element[0]]) {
      result[element[0]].push(element);
      result[element[0]].sort();
    } else {
...
export function createUser(login, password) {
  ...
  user.isWeakPassword = function checkPassword() {
    let hasUpperLetter = false;
    let hasLowerLetter = false;
    let lowSecurity = true;
    user.password.split('').forEach(element => {
    ...
    if (hasUpperLetter && hasLowerLetter) lowSecurity = false;
    return lowSecurity;
  }
  return user;
}
